function [time, sensor_data, force_data] = read_labview_file(filename)
    ...
    if has_header
        T = readtable(filename, 'Delimiter', '\t', 'HeaderLines', line_number, 'VariableNamingRule', 'preserve');
        ...
        T = readtable(filename, 'VariableNamingRule', 'preserve'); % Read normally if no header
        % Extract time and sensor readings
        time = T{:, 1} - min(T{:, 1}); % First column is time
