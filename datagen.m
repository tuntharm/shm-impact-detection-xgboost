...
for c = 1:length(case_folders)
    ...
    all_features = []; % Initialize correctly as 2D matrix
    ...
    for i = 1:length(file_list)
        ...
        all_features = [all_features; features(:)']; % Flatten & append as a row vector
    ...
    final_features = [distance_features, all_features, force_values];
    %%
    % Adjust column names to match grouped feature structure
     [num_sensors, num_features_per_sensor] = size(features);
...
% % output_folder = '/Users/tuntharm/Library/CloudStorage/OneDrive-ImperialCollegeLondon/Y4 work/FYP/FYP_Data/Processed_Data/';
% %----WINDOW----
% main_folder = "C:\Users\tunta\OneDrive - Imperial College London\Y4 work\FYP\FYP_Data\Raw_Data";
