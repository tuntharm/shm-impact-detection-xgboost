...
scatter(y_test(:,1), y_test(:,2), 100, 'b+', 'LineWidth', 2); % True positions as blue +
hold on;
scatter(y_pred_x, y_pred_y, 100, 'r+', 'LineWidth', 2); % Predicted positions as red +

% Draw Grey Dashed Lines Connecting True and Predicted Points
for i = 1:length(y_test)
    plot([y_test(i,1), y_pred_x(i)], [y_test(i,2), y_pred_y(i)], ...
...
x = [0, -width, -width, 0, 0]; % Right top corner at (0,0)
y = [0, 0, -height, -height, 0];

% Plot the square
plot(x, y, 'k-', 'LineWidth', 2); % Black square outline
...
for i = 1:num_h_lines
    y_h = -i * (height / (num_h_lines + 1));
    plot([-width, 0], [y_h, y_h], 'k-', 'LineWidth', 0.5); % White dashed lines
end

% Vertical grid lines
for j = 1:num_v_lines
    x_v = -j * (width / (num_v_lines + 1));
    plot([x_v, x_v], [-height, 0], 'k-', 'LineWidth', 0.5); % White dashed lines
end
...
for k = 1:length(S_labels)
    text(S_positions(k,1), S_positions(k,2), S_labels{k}, 'Color', 'b', ...
